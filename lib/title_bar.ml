...
let create ~project_name ~username = { project_name; username }
...
let image ~width (bar : t) =
  let width = Width.to_int width in
  let project_name = Project_name.to_string bar.project_name in
  let background = I.uchar bar_bg (Uchar.of_char ' ') width 1 in
  let left =
    I.(hpad 1 0 @@ string title_attrs (lamp_icon ^ " " ^ project_name))
  in
  let welcome =
    I.string welcome_attrs (Welcome.message bar.username)
  in
